  ...
  const NavItem = ({ id, label, icon, count }: { id: string, label: string, icon: React.ReactNode, count?: number }) => {
    ...
    return (
      <button
        ...
        className={`w-full flex items-center justify-between px-4 py-3 mb-1 text-sm rounded-lg transition-all duration-200 group ${
          isActive
            ? 'bg-lumbago-border/20 text-lumbago-primary border-l-4 border-l-lumbago-primary shadow-[0_0_15px_rgba(142,240,255,0.1)]'
            : 'text-lumbago-text-dim hover:bg-white/5 hover:text-white border-l-4 border-transparent'
        }`}
      >
        <div className="flex items-center truncate gap-3">
          <div className={`${isActive ? 'text-lumbago-primary drop-shadow-[0_0_5px_rgba(142,240,255,0.8)]' : 'text-slate-400 group-hover:text-white'}`}>
            {icon}
          </div>
        ...
        {count !== undefined && (
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
              isActive 
                ...
                : 'bg-lumbago-dark text-slate-400 border border-slate-700'
          }`}>
            {count}
          </span>
        )}
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="p-6 pb-2">
         <div className="flex items-center gap-3 mb-6">
            <div className="w-8 h-8 rounded bg-gradient-to-br from-lumbago-primary to-lumbago-secondary shadow-[0_0_10px_rgba(142,240,255,0.5)] flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-lumbago-dark" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3" /></svg>
            </div>
            <div className="flex flex-col">
                <span className="text-xl font-bold tracking-tight text-lumbago-primary drop-shadow-[0_0_8px_rgba(142,240,255,0.6)]">Lumbago</span>
                <span className="text-[10px] tracking-[0.2em] text-lumbago-secondary font-bold uppercase">Music AI</span>
            </div>
         </div>
      </div>

      <div className="flex-grow overflow-y-auto custom-scrollbar px-3 space-y-6">
        
        {/* Biblioteka */}
        <div>
            <h3 className="px-4 text-[11px] font-bold text-lumbago-primary/80 uppercase tracking-widest mb-2 flex items-center gap-2">
                <span className="w-1 h-1 rounded-full bg-lumbago-primary shadow-[0_0_5px_#8ef0ff]"></span> 
                Biblioteka
            </h3>
            <NavItem 
                id="library" 
        ...
        <div>
            <div className="flex items-center justify-between px-4 mb-2">
                <h3 className="text-[11px] font-bold text-lumbago-secondary/80 uppercase tracking-widest flex items-center gap-2">
                    <span className="w-1 h-1 rounded-full bg-lumbago-secondary shadow-[0_0_5px_#ff66cc]"></span>
                    Playlisty
                </h3>
            ...
            {isCreating && (
                <form onSubmit={handleCreateSubmit} className="mb-2 px-2 animate-fade-in">
                    <input
                        type="text"
                        autoFocus
                        placeholder="Nazwa..."
                        ...
                        className="w-full px-3 py-2 text-sm bg-lumbago-dark border border-lumbago-secondary/50 rounded text-white focus:ring-1 focus:ring-lumbago-secondary focus:outline-none placeholder-slate-600"
                    />
                </form>
            )}
                ...
                {playlists.map(playlist => (
                    <div key={playlist.id} className="group relative">
                        <button
                            ...
                            className={`w-full text-left px-4 py-2 text-sm transition-all truncate hover:bg-white/5 ${
                                activeView === `playlist:${playlist.id}`
                                ? 'text-white border-l-2 border-lumbago-secondary pl-[14px]'
        ...
        <div className="mt-4">
            <div className="flex items-center justify-between px-4 mb-2">
                <h3 className="text-[11px] font-bold text-lumbago-accent/80 uppercase tracking-widest flex items-center gap-2">
                    <span className="w-1 h-1 rounded-full bg-lumbago-accent shadow-[0_0_5px_#39ff14]"></span>
                    Smart Playlisty
                </h3>
                    ...
                    <div key={playlist.id} className="group relative">
                        <button
                            ...
                            className={`w-full text-left px-4 py-2 text-sm transition-all truncate hover:bg-white/5 ${
                                ...
                                ? 'text-lumbago-accent border-l-2 border-lumbago-accent pl-[14px]'
                                : 'text-slate-400 border-l-2 border-transparent'
                            }`}
                        >
        ...
        {/* Narzędzia */}
        <div className="mt-4">
            <h3 className="px-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                Narzędzia
            </h3>
            <NavItem 
                id="scan" 
            ...
            <NavItem 
                id="cratedigger" 
                label="Crate Digger 🔍" 
                icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" /></svg>}
            />
            <NavItem 
                id="organizer" 
